...
String name1="Automation Engineer"; //for Assertion ahead
...
Assert.assertEquals(nametx1, name1,"Hard Assertionerror Msg1 if Names not matching"); //Hardasser alternate test
System.out.println("Sysout: hardssert passed so No AssertionError");

SoftAssert sa= new SoftAssert();									//Softassert alternate test
sa.assertEquals(nametx1, "abc1","Soft Assertionerror Msg1 if Names not matching");
sa.assertEquals(nametx1, "abc2","Soft Assertionerror Msg2 if Names not matching");
sa.assertEquals(nametx1, "abc3","Soft Assertionerror Msg3 if Names not matching");
sa.assertAll();

System.out.println("If any asser fails, Testsuite is failed, so Secondary statements like Sysout, driver.quit will execute"); //wont work coz SA failed
driver.quit(); //wont work
}
else
{
...
Thread.sleep(5000);
driver.quit();	//wont work coz SA failed.
